import { SingleColorExample24, NonSingleColorExample16, BinaryColorExample24 } from '@univerjs/icons';
...
```tsx
import { SingleColorExample24 } from '@univerjs/icons';

function App() {
    return <SingleColorExample24 />;
}
```

你就会得到：

<SingleColorExample24 />

因为图标就是 React 组件，你可以给他传递任何 SVGElement 的属性，例如 style 属性：

```tsx
function App() {
    return <SingleColorExample24 style={{ color: 'red', fontSize: '22px' }} />;
}
```

<SingleColorExample24 style={{ color: 'red', fontSize: '22px' }} />
...
<p align="center">
    <NonSingleColorExample16 style={{ fontSize: '48px' }} />{' '}
</p>
...
function App() {
    return 
    <>
        <BinaryColorExample24 style={{ fontSize: '48px' }} extend={{ colorChannel1: 'red'}}/>
        <BinaryColorExample24 style={{ fontSize: '48px' }} extend={{ colorChannel1: 'blue'}}/>
    </>
}
```
<p align="center">
